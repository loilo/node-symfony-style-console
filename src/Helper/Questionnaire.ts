...
export default class Questionnaire {
    ...
    return new Promise<string>(resolve => {
      rl.on('line', (value: string) => {
        ...
        if (!validator || validator(value)) {
          ...
          this.output.error(
            typeof errorMsg === 'function' ? errorMsg(value) : errorMsg
          )
  ...
  public ask(
    ...
    return (
      this.doAsk({
        ...
        validator:
          validator || (hasDefaultValue ? null : Questionnaire.isFilled),
        errorMsg: 'A value is required.'
      })
        // Return default value if exists and input is empty
        .then(value =>
          !hasDefaultValue || Questionnaire.isFilled(value)
            ? value
            : defaultValue
        )
    )
  ...
  public confirm(question: string, defaultValue: boolean = true) {
    const formattedQuestion = ` <fg=green>${question} (yes/no)</> [<fg=yellow>${
      defaultValue ? 'yes' : 'no'
    }</>]`
